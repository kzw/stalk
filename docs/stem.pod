...
=item stem [-f -r -v -d -c <path> -a ]  <mount point>
...
with the following command.  If not mounting as root, modify I</etc/fstab> with
B<user> option

=over

=item mount -t fuse.Stalk <path/to/mount folder>
